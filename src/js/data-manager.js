...
class DataManager {
    ...
    async init() {
        ...
        try {
            await createDir('data', { dir: BaseDirectory.App, recursive: true });
        } catch (e) {
    ...
    async loadAllData() {
        for (const [key, filename] of Object.entries(this.DATA_FILES)) {
            try {
                const fileExists = await exists(`data/${filename}`, { dir: BaseDirectory.App });
                if (fileExists) {
                    const content = await readTextFile(`data/${filename}`, { dir: BaseDirectory.App });
                    this.DATA[key] = JSON.parse(content);
                } else {
                    // Create empty file
                    await writeTextFile(`data/${filename}`, JSON.stringify([], null, 2), { dir: BaseDirectory.App });
                    this.DATA[key] = [];
                }
            } catch (error) {
    ...
    saveState() {
        ...
        this.saveAllData().catch(console.error);
